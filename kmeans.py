...
def calc_sse(centroids: numpy.ndarray, labels: numpy.ndarray, data: numpy.ndarray):
    """
        For each centroid, find the distances between the data points of a centroid and the centroid,         and return the sum of squares of each distance. This value is Sum of Squared Errors.
    """
    distances = 0
...
class KMeans:
    ...
    def fit(self, data: numpy.ndarray):
        ...
        """
        self.centroid = self._init_centroid(data)# Initialize centroid using random method or KMeans++
        for _ in range(self.max_iter):# for as many iters as needed,
            distance = self._calc_distance(data)# Calculate distance between each data point and centroids
            cluster = self._assign_cluster(distance)# Assign cluster to data based on minimum distance to centroids
            new_centroid = self._update_centroid(data, cluster)# Get new centroids from all points in clusters
            diff = numpy.abs(self.centroid - new_centroid).mean()# Find net distances between old and new centroids
            self.centroid = new_centroid# set new centroids as current centroids

            if diff <= self.tolerance:
                break

        self.SSE = calc_sse(self.centroid, cluster, data)# Find SSE of centroids based on data.
